  ...
  useEffect(() => {
    const fetchSimulatorDetail = async () => {
      ...
      try {
        ...
        const found = allSimulators.find((sim: any) => (sim.SimID || sim.id)?.toString() === params.id);
        setSimulator(found);
  ...
  if (!simulator) {
    return (
      <div className="min-h-screen flex flex-col justify-center items-center gap-4 bg-gray-50">
        <h1 className="text-2xl font-bold text-gray-800">ไม่พบข้อมูล Simulator นี้</h1>
        <Link href="/hosting" className="text-orange-600 hover:underline font-medium">กลับไปหน้า My Simulators</Link>
      </div>
    );
  }

  const simName = simulator.SimListName || simulator.simListName;
  const simDesc = simulator.ListDescription || simulator.listDescription || simulator.ListDescripion || simulator.description;
  const simPrice = simulator.PricePerHour || simulator.pricePerHour || 0;
  const simImage = simulator.firstimage || simulator.firstImage;
